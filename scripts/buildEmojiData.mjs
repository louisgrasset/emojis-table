...
import descriptions from "emojilib" assert {type: "json"};
...
const data = Object.entries(emojis).map(([emoji, data], index) => {
    ...
    return {
        ...data,
        emoji: emoji,
        description: descriptions[emoji] ? [data.name, ...descriptions[emoji]] : [data.name]
    }
});
...
writeFileSync(`${filePath}/emojidb-raw.json`, JSON.stringify(data), {encoding: 'utf-8'});
writeFileSync(`${filePath}/emojidb.json`, JSON.stringify(dataByGroup), {encoding: 'utf-8'});
